 % Om = 0, HIT = 1, CR = 2, FA = 3
    ...
    cd('/scratch/work/bahriz1/Thesis/rerun_downloads')
    ratId = '8582';
    date = '121018';
    %missingIdx = 52;
    foldername = [ratId '_' date];
    cd(foldername);
    %%
    gngFile = dir('*.mat');
    nevFile = dir('*.nev');
    ...
    EventsName = nevFile(1).name;


    %%

    Times = GetEventsData_linux(EventsName, 0, 'microsec');
    data = load(gngFile(1).name);
    ...
    %% For files with miss-matching length run this cell not the next one
    % s = Times.InstruResp(:);
    % t = T.StimOn(respMask);
    % 
    % 
    % avgDiff = mean(s(1:missingIdx-1) - t(1:missingIdx-1));
    % 
    % inspector = s(1:missingIdx) - t(1:missingIdx);
    % newResp = [Times.InstruResp(1:missingIdx-1) t(missingIdx)+avgDiff Times.InstruResp(missingIdx:end)]';
    % T.InstruResp(respMask) = newResp(:);
    %%
    T.InstruResp(respMask) = Times.InstruResp(2:end);
    %%
    ...
    savePath = fullfile('/scratch/work/bahriz1/Thesis/data/ratId_date', fileName);
    save(savePath, 'T');
